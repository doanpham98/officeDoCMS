...
const Product = (product) => {
  ...
  const onDeleteButton =()=>{
    ...
    return <tr>
        <td>{product.id}</td>
        <td>{product.name}</td>
        <td>{product.price}  <sup>vnđ</sup></td>
        <td><img src={product.image} height="100px" alt="product"/></td>
        <td className="button-group"><Button color="secondary" onClick={toggleEdit}><FontAwesomeIcon icon={faEdit}/> Edit</Button>
        ...
        <Button color="danger" onClick={toggleDelete}><FontAwesomeIcon icon={faTrashAlt}/>  Delete</Button></td>
